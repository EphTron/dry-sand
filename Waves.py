...
class Waves:
  def __init__(self, ISLAND_POS, SCREEN_SIZE, TRANS_COLOR):
    ...
    self.wave_space = self.SCREEN_SIZE.height * ISLAND_POS
    self.wave_list = []
  ...
  def fill_wave_space(self):
    ...
    _parts_needed = 0
    if (self.SCREEN_SIZE.width - self.img_width) <= 0:
      _parts_needed = 2
    elif (self.SCREEN_SIZE.width - self.img_width) > 0:
      _p = math.ceil(self.SCREEN_SIZE.width / self.img_width)
      _parts_needed = _p + 1
    ...
    for i in range(0, int(_waves_needed+1)):
      ...
      _wave = Wave(i, self.img, self.img_width, _parts_needed, Vec2(_x, _y))
  ...
  def update(self):
    for wave in self.wave_list:
      ...
      if (wave.position.x + wave.IMG_WIDTH < 0):
        wave.set_x(0)
